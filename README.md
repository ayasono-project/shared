...
各アプリの `package.json` から **GitHub Release の prebuilt tarball URL** で取り込みます。タグ（`v*`）を push すると CI（`.github/workflows/release.yml`）が build/test → `pnpm pack`（dist 同梱）→ Release に `.tgz` を添付します。consumer はこの `.tgz` を参照するため、**インストール時ビルド（prepare/tsup）も認証も不要**です。

```json
{
  "dependencies": {
    "@ayasono/shared": "https://github.com/ayasono-project/shared/releases/download/v0.2.1/ayasono-shared-0.2.1.tgz"
  }
}
```
...
配布を **GitHub Release の prebuilt tarball + タグ駆動 CI** に切替（`pnpm pack` 成果物を Release 添付）。core の内容は v0.2.0 と同一。
