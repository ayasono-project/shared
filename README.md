...
{
  "dependencies": {
    "@ayasono/shared": "github:ayasono-project/shared#v0.2.3"
  }
}
```
...
release CI の dist stale 検出を `git status --porcelain` に修正（`git diff --exit-status` が runner で usage エラーになっていた）。配布方式・core 内容は v0.2.2 と同一。
