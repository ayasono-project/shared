// tests/core/customErrors.test.ts
// カスタムエラークラス（BaseError 階層）のプロパティ・継承・運用フラグ挙動を検証する
...
describe("CustomErrors", () => {
  ...
  describe("ValidationError", () => {
    ...
    it("fromKey で遅延翻訳用の messageKey / messageParams が設定されること", () => {
      const error = ValidationError.fromKey(
        "some.key",
        { count: 3 },
        "タイトル",
      );
      ...
      expect(error).toBeInstanceOf(ValidationError);
      expect(error.message).toBe("some.key");
      expect(error.messageKey).toBe("some.key");
      expect(error.messageParams).toEqual({ count: 3 });
      expect(error.embedTitle).toBe("タイトル");
    });
  });
