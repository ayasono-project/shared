// tests/core/discordWebhookTransport.test.ts
// DiscordWebhookTransport の Webhook 送信・エラー耐性・description 組み立てロジックを検証する

import type { Mock } from "vitest";
import { DiscordWebhookTransport } from "../../src/core/discordWebhookTransport";

const TEST_WEBHOOK_URL = "https://discord.com/api/webhooks/123/token";

// getTitle で注入するタイトル（i18n 解決はアプリ側責務のため、テストでは固定文字列を渡す）
const TEST_TITLE = "🚨 saika エラー通知";

// Discord Embed の description 文字数上限
const MAX_DESC_LENGTH = 4096;

const createTransport = (): DiscordWebhookTransport =>
  new DiscordWebhookTransport(TEST_WEBHOOK_URL, { getTitle: () => TEST_TITLE });

describe("core/discordWebhookTransport", () => {
  let fetchMock: Mock;
  ...
  it("タイトル・カラー・タイムスタンプを含む正しい Embed ペイロードを送信すること", () => {
    ...
    expect(body.embeds[0]?.title).toBe(TEST_TITLE);
  ...
  it("message が文字列以外（数値）の場合に String() で変換されること", () => {
    const transport = createTransport();
    const callback = vi.fn();
  ...
  it("message が undefined の場合に空文字列へフォールバックすること", () => {
    const transport = createTransport();
    const callback = vi.fn();
